...
// --- CẬP NHẬT THÔNG TIN CÁ NHÂN THỦ THƯ ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    $fullname = trim($_POST['fullname']);
    $email = trim($_POST['email']);
    $phone = trim($_POST['phone']);
    
    $stmt = $pdo->prepare("UPDATE Users SET major_name = ?, email = ?, phone_number = ? WHERE username = ?");
    $stmt->execute([$fullname, $email, $phone, $_SESSION['username']]);
    $_GET['profile_msg'] = 'updated';
}

// --- TRUY XUẤT THÔNG TIN THỦ THƯ ---
...
// --- ĐỔI MẬT KHẨU CHO THỦ THƯ ---
$change_msg = '';
if ($tab === 'account' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['old_password'], $_POST['new_password'], $_POST['confirm_password'])) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['book_action'])) {
    if ($_POST['book_action'] == 'add') {
        ...
        $stmt = $pdo->prepare("INSERT INTO Books (title, author, category, quantity, image_url) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$title, $author, $cat, $qty, $image_url]);
    } 
    elseif ($_POST['book_action'] == 'edit') {
        ...
        $oldImage = $oldBook ? $oldBook['image_url'] : null;
        $image_url = uploadBookImage('cover_image', $oldImage);

        $stmt = $pdo->prepare("UPDATE Books SET title=?, author=?, category=?, quantity=?, image_url=? WHERE id=?");
        $stmt->execute([$title, $author, $cat, $qty, $image_url, $b_id]);
    }
...
// --- NGHIỆP VỤ 2: DUYỆT / TỪ CHỐI / TRẢ SÁCH ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['req_action'])) {
    ...
    if ($action == 'approve' && $req['status'] == 'pending') {
        ...
        $stmt->execute([$req['book_id']]);
        $bookQty = (int)$stmt->fetchColumn();

        if ($bookQty >= (int)$req['quantity']) {
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("UPDATE BorrowRequests SET status='approved' WHERE id=?");
            $stmt->execute([$req_id]);

            $stmt = $pdo->prepare("UPDATE Books SET quantity = quantity - ? WHERE id=?");
            $stmt->execute([$req['quantity'], $req['book_id']]);
            $pdo->commit();
        }
    } 
    elseif ($action == 'reject' && $req['status'] == 'pending') {
        $reason = trim($_POST['reject_reason']);
        $stmt = $pdo->prepare("UPDATE BorrowRequests SET status='rejected', reject_reason=? WHERE id=?");
        $stmt->execute([$reason, $req_id]);
    }
    elseif ($action == 'return' && $req['status'] == 'approved') {
        $today = date('Y-m-d');
        $late_fee = 0;
        
        $diff_days = (strtotime($today) - strtotime($req['return_date'])) / 86400;
        if ($diff_days > 0) {
            $late_fee = $diff_days * 10000;
        }

        $pdo->beginTransaction();
        $stmt = $pdo->prepare("UPDATE BorrowRequests SET status='returned', actual_return_date=?, late_fee=? WHERE id=?");
        $stmt->execute([$today, $late_fee, $req_id]);
        $pdo->exec("UPDATE Books SET quantity = quantity + {$req['quantity']} WHERE id={$req['book_id']}");
        $pdo->commit();
    }
    header("Location: indexlibrarian.php?tab=requests"); exit;
}

// --- XỬ LÝ LẤY SÁCH ĐỂ SỬA ---
$edit_book = null;
...
// ==========================================
// LOGIC PHÂN TRANG & TÌM KIẾM SÁCH
// ==========================================
$limit = 32; // 32 cuốn sách mỗi trang
...
// Lấy danh sách yêu cầu mượn kèm thông tin liên hệ của sinh viên
$reqs = $pdo->query("SELECT br.*, b.title, u.phone_number AS phone, u.email FROM BorrowRequests br JOIN Books b ON br.book_id = b.id JOIN Users u ON br.username = u.username ORDER BY br.status ASC, br.id DESC")->fetchAll();
?>
...
<head>
    ...
    <style type="text/css">
        ...
        .pagination .page-item.active .page-link { background-color: var(--qnu-blue); border-color: var(--qnu-blue); }
        
        footer { background: #0f172a; color: #fff; padding: 30px 0 0 0; border-top: 4px solid var(--qnu-gold); margin-top: auto; }
    </style>
</head>
    ...
    <header class="qnu-header sticky-top">
        <div class="container-fluid px-4 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center">
                <span class="me-3 fw-bold text-primary small"><i class="bi bi-person-badge-fill fs-5 align-middle me-1"></i> <?php echo $_SESSION['username']; ?></span>
            ...
            <div class="col-lg-2 mb-4">
                <div class="content-card p-3 sticky-top" style="top: 100px;">
                    <div class="nav flex-column nav-pills">
                        <a href="?tab=books" class="nav-link <?php echo $tab=='books'?'active':''; ?>">
                            <i class="bi bi-journal-album me-2"></i> Quản lý sách
                        </a>
                        <a href="?tab=requests" class="nav-link <?php echo $tab=='requests'?'active':''; ?>">
                            <i class="bi bi-clipboard-check me-2"></i> Duyệt Mượn / Trả
                        </a>
                        <hr class="text-muted opacity-25">
                        <a href="?tab=profile" class="nav-link <?php echo $tab=='profile'?'active':''; ?>">
                            <i class="bi bi-person-circle me-2"></i> Tài khoản cá nhân
                        </a>
                        <a href="?tab=account" class="nav-link <?php echo $tab=='account'?'active':''; ?>">
            ...
            <div class="col-lg-10">
                ...
                <div class="content-card mx-auto" style="max-width: 650px;">
                    ...
                    <form method="POST" action="?tab=profile">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="fullname" class="form-control" value="<?php echo htmlspecialchars($librarian_info['major_name'] ?? ''); ?>" placeholder="Nhập họ tên" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($librarian_info['email'] ?? ''); ?>" placeholder="Nhập email" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="tel" name="phone" class="form-control" value="<?php echo htmlspecialchars($librarian_info['phone_number'] ?? ''); ?>" placeholder="Nhập số điện thoại">
                            </div>
                        </div>
                ...
                <div class="content-card mx-auto" style="max-width: 600px;">
                    ...
                    <form method="POST" action="?tab=account">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">Mật khẩu cũ</label>
                    ...
                    <div class="col-lg-3 mb-4">
                        <div class="content-card">
                            ...
                            <form method="POST" action="?tab=books" enctype="multipart/form-data">
                                ...
                                <div class="mb-2">
                                    <label class="form-label small text-muted mb-1">Tên sách</label>
                                    <input type="text" name="title" class="form-control form-control-sm" value="<?php echo $edit_book ? htmlspecialchars($edit_book['title']) : ''; ?>" required>
                                </div>
                                <div class="mb-2">
                                    <label class="form-label small text-muted mb-1">Tác giả</label>
                                    <input type="text" name="author" class="form-control form-control-sm" value="<?php echo $edit_book ? htmlspecialchars($edit_book['author']) : ''; ?>" required>
                                </div>
                                <div class="mb-2">
                                    <label class="form-label small text-muted mb-1">Thể loại</label>
                                    <select name="category" class="form-select form-select-sm">
                                        <option value="Khác">-- Chọn thể loại --</option>
                                        <option value="Công nghệ thông tin" <?php echo ($edit_book && $edit_book['category']=='Công nghệ thông tin')?'selected':'';?>>Công nghệ thông tin</option>
                                        <option value="Kỹ thuật xây dựng" <?php echo ($edit_book && $edit_book['category']=='Kỹ thuật xây dựng')?'selected':'';?>>Kỹ thuật xây dựng</option>
                                        <option value="Kinh tế" <?php echo ($edit_book && $edit_book['category']=='Kinh tế')?'selected':'';?>>Kinh tế</option>
                                        <option value="Ngoại ngữ" <?php echo ($edit_book && $edit_book['category']=='Ngoại ngữ')?'selected':'';?>>Ngoại ngữ</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label small text-muted mb-1">Số lượng</label>
                                    <input type="number" name="quantity" class="form-control form-control-sm" value="<?php echo $edit_book ? $edit_book['quantity'] : ''; ?>" min="0" required>
                                </div>
                                <div class="mb-3">
                                ...
                                <?php if ($edit_book && !empty($edit_book['image_url'])): ?>
                                    <div class="mb-3">
                                        ...
                                        <img src="<?php echo htmlspecialchars($edit_book['image_url']); ?>" 
                                ...
                                <?php if($edit_book): ?>
                                    <a href="?tab=books" class="btn btn-secondary w-100 btn-sm mt-2">Hủy</a>
                                <?php endif; ?>
                            </form>
                        </div>
                    </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <form method="GET" class="d-flex" style="width: 350px;">
                                    ...
                                    <div class="input-group input-group-sm">
                                        ...
                                        <?php if($search): ?>
                                            <a href="?tab=books" class="btn btn-outline-secondary"><i class="bi bi-x-lg"></i></a>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
                                        ...
                                        <?php if(count($books) > 0): ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    ?>
                                                    <img src="<?php echo htmlspecialchars($cover); ?>" style="width:45px; height:65px; object-fit:cover; border-radius:5px;">
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <?php else: ?>
                                                        <span class="badge bg-success"><?php echo $b['quantity']; ?> cuốn</span>
                                                    <?php endif; ?>
                                                </td>
                                                <td class="text-center">
                                                    ...
                                                    <form method="POST" style="display:inline;">
                                                        ...
                                                        <button type="submit" name="book_action" value="delete" class="btn btn-sm btn-outline-danger" onclick="return confirm('Xóa sách này?');" title="Xóa"><i class="bi bi-trash"></i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                            <?php endforeach; ?>
                            ...
                            <nav class="mt-3">
                                <ul class="pagination pagination-sm justify-content-center mb-0">
                                    <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                                        <a class="page-link" href="?tab=books&search=<?php echo urlencode($search); ?>&page=<?php echo $page-1; ?>">Trước</a>
                                    </li>
                                    ...
                                    ?>
                                        <li class="page-item <?php echo ($i == $page) ? 'active' : ''; ?>">
                                            <a class="page-link" href="?tab=books&search=<?php echo urlencode($search); ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                                        </li>
                                    <?php endfor; ?>

                                    <li class="page-item <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>">
                                        <a class="page-link" href="?tab=books&search=<?php echo urlencode($search); ?>&page=<?php echo $page+1; ?>">Sau</a>
                                    </li>
                                </ul>
                            </nav>
                            <?php endif; ?>
                ...
                <?php if($tab == 'requests'): ?>
                <div class="content-card">
                    <h4 class="fw-bold text-dark mb-4"><i class="bi bi-card-checklist text-primary"></i> Quản lý Yêu cầu Mượn & Trả Sách</h4>
                    <div class="table-responsive">
                            ...
                            <tbody>
                                <?php if(count($reqs) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="fw-bold text-primary text-center"><?php echo htmlspecialchars($r['username']); ?></td>
                                        <td style="font-size: 13px;">
                                            <div class="d-flex flex-column gap-1">
                                                <?php if($r['email']): ?>
                                                    <a href="mailto:<?php echo htmlspecialchars($r['email']); ?>" class="text-decoration-none" title="Gửi email"><i class="bi bi-envelope-fill text-danger"></i> <?php echo htmlspecialchars($r['email']); ?></a>
                                                <?php else: ?>
                                                    <span class="text-muted"><i class="bi bi-envelope-fill"></i> N/A</span>
                                                <?php endif; ?>
                                                <?php if($r['phone']): ?>
                                                    <a href="tel:<?php echo htmlspecialchars($r['phone']); ?>" class="text-decoration-none" title="Gọi điện"><i class="bi bi-telephone-fill text-success"></i> <?php echo htmlspecialchars($r['phone']); ?></a>
                                                <?php else: ?>
                                                    <span class="text-muted"><i class="bi bi-telephone-fill"></i> N/A</span>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                        <td style="max-width:200px;" class="text-truncate" title="<?php echo htmlspecialchars($r['title']); ?>"><?php echo htmlspecialchars($r['title']); ?></td>
                                        <td class="text-center"><?php echo $r['quantity']; ?></td>
                                        <td class="text-center">
                                            <?php echo date('d/m/Y', strtotime($r['return_date'])); ?>
                                            <?php if($r['total_fee'] > 0): ?>
                                                <br><small class="text-muted">Phí: <?php echo number_format($r['total_fee']); ?>đ</small>
                                            <?php endif; ?>
                                        </td>
                                        <td class="text-center">
                                            ...
                                            <?php elseif($r['status'] == 'approved'): ?>
                                                ...
                                                <?php if($is_overdue): ?><br><span class="badge bg-danger mt-1">Trễ hạn</span><?php endif; ?>
                                            <?php elseif($r['status'] == 'rejected'): ?>
                                                <span class="badge bg-danger">Từ chối</span>
                                                <?php if($r['reject_reason']): ?><br><small class="text-muted"><?php echo htmlspecialchars($r['reject_reason']); ?></small><?php endif; ?>
                                            <?php elseif($r['status'] == 'returned'): ?>
                                                <span class="badge bg-secondary">Đã trả</span>
                                                <?php if($r['late_fee'] > 0): ?><br><span class="text-danger small fw-bold">Phạt: <?php echo number_format($r['late_fee']); ?>đ</span><?php endif; ?>
                                            <?php endif; ?>
                                        </td>
                                        <td class="text-center">
                                            <?php if($r['status'] == 'pending'): ?>
                                                <div class="d-flex justify-content-center gap-1 flex-wrap">
                                                    <form method="POST"><input type="hidden" name="req_id" value="<?php echo $r['id']; ?>">
                                                        <button type="submit" name="req_action" value="approve" class="btn btn-sm btn-success py-0" title="Duyệt"><i class="bi bi-check-lg"></i></button>
                                                    </form>
                                                    <form method="POST" class="d-flex gap-1">
                                                        <input type="hidden" name="req_id" value="<?php echo $r['id']; ?>">
                                                        <input type="text" name="reject_reason" placeholder="Lý do..." required class="form-control form-control-sm py-0" style="width:70px;">
                                                        <button type="submit" name="req_action" value="reject" class="btn btn-sm btn-danger py-0" title="Từ chối"><i class="bi bi-x-lg"></i></button>
                                                    </form>
                                                </div>
                                            <?php elseif($r['status'] == 'approved'): ?>
                                                <form method="POST">
                                                    <input type="hidden" name="req_id" value="<?php echo $r['id']; ?>">
                                                    <button type="submit" name="req_action" value="return" class="btn btn-sm btn-primary py-0" onclick="return confirm('Sinh viên đã hoàn trả sách?');"><i class="bi bi-arrow-return-left"></i> Thu hồi</button>
                                                </form>
                                            <?php else: ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="8" class="text-center py-4 text-muted">Chưa có yêu cầu mượn sách nào.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
